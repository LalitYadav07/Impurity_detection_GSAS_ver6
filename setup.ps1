...
try {
    ...
    # GSAS-II binaries require psutil
    # numpy < 2.0 limitation must be removed for recent Python/GSAS-II versions
    pixi add psutil
    
    Write-Host "Building GSAS-II binaries (this requires a C++/Fortran compiler)..."
    Write-Host "NOTE: This step compiles Fortran/C code and can take 15-30 minutes with NO visible output." -ForegroundColor Yellow
    Write-Host "      Do NOT close this window. If it appears frozen, it is still compiling." -ForegroundColor Yellow
