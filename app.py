...
def check_gsas_installation():
    try:
        ...
        st.error(f"⚠️ GSAS-II initialization failed: {e}")
        return False
...
def render_file_explorer(path: Path, key_prefix: str, filter_exts=None, depth=0):
    ...
    for item in items:
        ...
        if item.is_dir():
            ...
            # Pre-check if folder has any matching files before rendering expander
    ...
    if st.session_state.pipeline_process and st.session_state.log_queue:
        ...
        if process.poll() is not None and q.empty():
            ...
            if process.returncode == 0:
                ...
                st.error(f"❌ Pipeline failed (exit code {process.returncode}). Check pipeline.log for details.")
...
# --- UI HEADER ---
st.title("🔬 Phase Detection for Powder Diffraction")
if IS_HF_SPACES:
    st.info("🚀 **Hugging Face Spaces Detected**: Resource limits (max 2 workers) are active to prevent OOM crashes.")
st.markdown("**R**esidual-**A**ware **D**eep-learning–**A**ssisted **R**efinement for **P**owder **D**iffraction — Automated phase discovery using ML-guided Rietveld refinement.")
...
with st.sidebar:
    ...
    st.markdown("### ☢️ Radiation Source")
...
# --- TABS ---
t_run, t_res, t_int, t_exp = st.tabs(["🚀 Run & Progress", "📊 Results", "📈 Interactive Plots", "📂 Run File Browser"])
    ...
    with c1:
        ...
        def render_pipeline_progress():
            if st.session_state.run_active or st.session_state.run_finished:
                ...
                st.info("Start a run to see live pipeline progress.")
...
with t_int:
    ...
    st.caption("Plotly-rendered interactive versions of generated artifacts (when plot payload sidecars are available).")
    ...
    if not INTERACTIVE_PLOTS_AVAILABLE:
        ...
        def render_interactive_plots():
            ...
            if not payload_files:
                ...
                st.caption(f"Found {len(payload_files)} interactive payload file(s).")
                for payload_path in payload_files:
                    try:
                        rel = payload_path.relative_to(rdir)
                        title = str(rel)
                    except Exception:
                        title = str(payload_path)
                    ...
                    with st.expander(f"🧩 {title}", expanded=False):
                        try:
                            ...
                            if src_plot and Path(src_plot).exists():
                                st.caption(f"Source static plot: {src_plot}")
                        except Exception as e:
...
with t_exp:
    ...
    if st.session_state.run_dir:
        ...
        if not rdir.exists():
            ...
            if not lfile.exists():
                st.warning("⚠️ pipeline.log is not available in the run directory yet.")
