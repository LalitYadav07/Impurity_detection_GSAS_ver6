...
DEFAULT_DB_ROOT = PROJECT_ROOT / "data" / "database_xray"

CATALOG = os.environ.get("CATALOG", str(DEFAULT_DB_ROOT / "catalog_deduplicated.csv"))
METAJS  = os.environ.get("METAJS",  str((PROJECT_ROOT / "data" / "database_neutron" / "highsymm_metadata.json")))
INDEX   = os.environ.get("INDEX",   str(DEFAULT_DB_ROOT / "profiles64" / "index.csv"))
NPZ     = os.environ.get("NPZ",     str(DEFAULT_DB_ROOT / "profiles64" / "profiles64.npz"))

# ===================== Config =====================
PCT = float(os.environ.get("PCT", "5"))  # percent tol for a,b,c,alpha,beta,gamma
PCT_VOLUME = 3.0 * PCT                   # use a wider tolerance for volume
...
def identical_pair_by_cif(r1, r2):
    ...
    # Use a wider tolerance for V; base tolerance for the remaining fields.
    for k in FIELDS:
...
logger.info(f"[1/5] Loading catalog: {CATALOG}")
with open(CATALOG, "r", encoding="utf-8") as f:
    cat_rows = list(csv.DictReader(f))
logger.info(f"  rows in catalog: {len(cat_rows):,}")

id_to_catrow = {r["id"]: r for r in cat_rows if r.get("id")}
cat_ids = list(id_to_catrow.keys())

logger.info(f"[2/5] Loading metadata JSON: {METAJS}")
...
for rid in cat_ids:
    ...
    if cif_sg is None:
        try:
            cif_sg = int(rec.get("space_group"))
        except Exception:
            pass
...
logger.info(f"  usable with CIF+catalog SG: {len(usable):,}  | skipped no meta: {skipped_no_meta:,}  | skipped missing fields: {skipped_missing_fields:,}")

# ===================== Blocking + clustering (with catalog SG safety) =====================
logger.info("[3/5] Clustering duplicates (CIF rule + catalog SG safety)…")
...
for key, group in buckets.items():
    ...
    for i in range(n):
        r1 = group_sorted[i]
        j = i + 1
        # Candidate window on V uses the volume tolerance.
...
logger.info(f"[4/5] Updating profiles index & NPZ…")
if not os.path.exists(INDEX):
    logger.warning("  profiles index not found; skipping index & NPZ steps.")
    idx_rows = []
else:
    with open(INDEX, "r", encoding="utf-8") as f:
        idx_rows = list(csv.DictReader(f))
    logger.info(f"  index rows: {len(idx_rows):,}")
...
# ===================== Console summary =====================
logger.info("\n----- DONE -----")
logger.info(f"Deduped catalog:   {OUT_CAT}")
logger.info(f"Log (CSV):         {OUT_LOG}")
logger.info(f"Summary (TXT):     {OUT_SUM}")
if 'idx_rows' in locals() and idx_rows:
    logger.info(f"Index (drop-only): {OUT_INDEX_DROPSAFE}")
if 'made_npz' in locals() and made_npz:
    logger.info(f"New NPZ:           {OUT_NPZ}")
    logger.info(f"Remapped index:    {OUT_INDEX_REMAPPED}")
logger.info(f"Catalog removed {removed_cat:,} / kept {kept_cat:,}")
if 'idx_rows' in locals() and idx_rows:
    logger.info(f"Index   removed {removed_idx:,} / kept {kept_idx:,}")
