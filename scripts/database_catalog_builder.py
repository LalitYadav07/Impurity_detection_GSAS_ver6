...
def process_one_phase(phase_id: str, meta: Dict,
    ...
    try:
        ...
        # Top-M reflections
        hkls, d0, q0, I0 = simulate_topM_peaks(s, two_theta_min, two_theta_max, topM, radiation, wavelength)
        if hkls.shape[0] == 0:
            raise RuntimeError(f"No peaks returned by diffraction calculator (radiation={radiation})")
...
def main():
    ...
    ap.add_argument("--wavelength", type=float, default=1.54184, help="Wavelength in Angstroms (xray only relevant)")
