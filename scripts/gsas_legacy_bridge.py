...
def _knee_keep_ids(items, id_fn, val_fn, *, label:str,
    ...
    n = len(rows)
    if n == 0:
        logger.info(f"[KNEE] {label}: no finite values → keep 0/0")
        return []
    ...
    def _fallback(reason):
        ...
        logger.info(f"[KNEE] {label}: {reason} (n={n}, span≈{span:.4g}) → keep {len(kept)}/{n} { _fmt_list(kept, [v for _,v in rows]) }")
        return kept
    ...
    logger.info(f"[KNEE] {label}: n={n}, span≈{span:.4g}, knee@idx={imax} (rank={imax+1}, cut≈{thr:.4g}) → keep {len(kept_ids)}/{n} {_fmt_list(kept_ids, kept_vals)}")
    return kept_ids
...
def _apply_knee_union_for_ml(items: List[CandidatePhase], stage_label: str, kcfg: Dict[str, Any]) -> List[str]:
    ...
    ordered = sorted(set(union_ids),
                     ...
                     reverse=True)
    logger.info(f"[KNEE] {stage_label}/UNION (RRF): selected {len(ordered)} → {_fmt_list(ordered)}")
    return ordered
...
class LegacyPipelineBridge:
    ...
    def set_element_constraints(self, allowed_elements: List[str], stable_ids: Optional[set] = None):
        ...
        logger.info(f"Element constraints set: {allowed_elements}")
        logger.info(f"Element mask built successfully: Hi={int(M_hi)}, Lo={int(M_lo)}")
    ...
    def filter_candidates_by_elements(self, candidate_ids: List[str]) -> List[str]:
        ...
        logger.info(f"Element filtering: {total} → {kept} candidates (rejected {total - kept})")

        preview = kept_ids[:5]
        if preview:
            logger.info("Element filter: examples (id, name, SG):")
            for pid in preview:
                name, sg = self._display_name_sg(pid)
                logger.info(f"  - {pid}: {name}, SG={sg}")
    ...
    def filter_candidates_by_space_group(self, candidate_ids: List[str],
        ...
        logger.info(f"Space-group pruning (exclude {list(exclude_sg)}): {len(candidate_ids)} → {len(kept_ids)} candidates")

        preview = kept_ids[:5]
        if preview:
            logger.info("Space-group stage: examples (id, name, SG):")
            for pid in preview:
                name, sg = self._display_name_sg(pid)
                logger.info(f"  - {pid}: {name}, SG={sg}")
        ...
        try:
            ...
            if hasattr(self.db_loader, "has_stable_catalog") and callable(self.db_loader.has_stable_catalog):
                ...
                if not self.db_loader.has_stable_catalog():
                    logger.info("[stable] no stable catalog attached → skipping stability filter")
                    return candidate_ids

            if not stable_ids:
                logger.info("[stable] stable id set is empty → skipping stability filter")
                return candidate_ids
            ...
            if not kept:
                logger.info(f"[stable] none of {len(candidate_ids)} candidates are in stable list → leaving list unchanged")
                return candidate_ids

            logger.info(f"[stable] stability pruning: {len(candidate_ids)} → {len(kept)} candidates")
            # preview a few
            for pid in kept[:5]:
                try:
                    name, sg = self._display_name_sg(pid)
                    logger.info(f"  - {pid}: {name}, SG={sg}  (stable)")
                except Exception:
                    pass
            return kept

        except Exception as e:
            logger.warning(f"[stable] stability filter encountered an error → skipping. reason: {e}")
            return candidate_ids
    ...
    def dedup_by_hist_and_elements(
        ...
        *,
        ...
        for pid, base, r in dropped_log[:12]:
            ...
            logger.info(f"  [dup] drop {pid} ({n1}, SG={sg1}) ~ {base} ({n2}, SG={sg2}); r={r:.3f}")
        if len(dropped_log) > 12:
            logger.info(f"  ... and {len(dropped_log)-12} more")
    ...
    def screen_by_histogram(self,
        ...
        logger.info(f"[hist-ml] using topN(selection)={selection_topN}  plot_top_k={plot_top_k}")
        ...
        logger.info(f"[hist-ml] passed={len(passed)} / details={len(details)}; plot_top_k={plot_top_k}")
        ...
        logger.info(f"Histogram screening: {len(candidate_ids)} → {len(scored)} candidates")
        ar = meta.get("active_range")
        logger.debug(f"Histogram meta: active_bins={meta.get('active_bins')}, active_range={ar}, sum_residual={meta.get('sum_residual'):.3g}")
        ...
        if scored:
            ...
            for pid, sc in scored:
                ...
                logger.info(f"  - {pid}: {name}, SG={sg}, score={sc:.3f} (cos={cos:.3f}, α={alpha:.3f}{p_str}, explained={R_cov:.3f})")
            import sys
            sys.stdout.flush()
...
class IntegratedCandidateScreener:
    ...
    def screen_candidates_comprehensive(self,
        ...
        """
        logger.info("=== Comprehensive Candidate Screening (ML-only) ===")

        # 1) element mask
        logger.info(f"Starting with {len(all_candidate_ids)} total candidates")
        ...
        logger.info(f"After element filtering: {len(candidates)} candidates")
        ...
        # 4) histogram (ML)
        logger.info("Running histogram screening (ML)...")
        ...
        logger.info(f"Final screening: {len(all_candidate_ids)} → {len(final_candidates)} candidates")
        logger.info("Top candidates selected:")
        for i, c in enumerate(final_candidates[:5], 1):
            logger.info(f"  {i}. {c.phase_id}: score={c.histogram_score:.3f}")
...
"""
    ...
    logger.info(f"[stage0] wrote dummy CIF → {outp}")
    return str(outp)
...
def stage0_bootstrap_no_cif(
    ...
    *,
    ...
    try:
        ...
        logger.info("[stage0] 0-cycle compute complete: residual ≈ Yobs")
    except Exception as e:
        logger.debug(f"[stage0] 0-cycle compute skipped (non-fatal): {e}")
    ...
    try:
        ...
        if rows:
            ...
            logger.info(f"[stage0] histogram summary saved: {csv_path}")
    except Exception as e:
        logger.warning(f"[stage0] failed to save histogram summary: {e}")

    logger.info(f"[stage0] sending top-{len(phase_ids)} to nudger: {phase_ids}")
    ...
    logger.info(f"[stage0] selected main phase → id={main_pid}, cif={main_cif}")
    return str(main_cif), main_pid, s4_res

def test_legacy_bridge():
    logger.info("GSAS-II Legacy Bridge (clean, ML-only) ready. Key stages: Element mask integration; Space-group pruning; 64-bin histogram screening; Dedup by SG+elements with full 64-bin Pearson")
