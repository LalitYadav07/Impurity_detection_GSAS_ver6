...
# Monkey-patch wx.App to avoid crash on headless systems
try:
    import wx
    class MockApp:
        def __init__(self, *args, **kwargs):
            # Do NOT call super().__init__ as it initializes C++ GUI which crashes
            global _global_mock_app
            _global_mock_app = self
            
        def MainLoop(self):
            pass
            
        def GetMainTopWindow(self):
            return None
    ...
    _global_mock_app = None
    
    def MockGetApp():
        return _global_mock_app
        
    wx.App = MockApp
    wx.GetApp = MockGetApp
except ImportError:
    pass

try:
    import wx
    # Create the mock app immediately if not present
    if not wx.GetApp():
        app = wx.App(False)
except Exception:
    pass
