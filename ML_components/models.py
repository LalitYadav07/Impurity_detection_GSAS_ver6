        ...
        try:
            ...
            for ax, d in zip(axes, details[:n_show]):
                ...
                candidate_curves.append({
                    "phase_id": str(d["phase_id"]),
                    "label": _lbl(d["phase_id"]),
                    "score": float(d["score"]),
                    "alpha": float(d["alpha"]),
                    "cosine": float(d["cosine"]),
                    "present_prob": float(d.get("present_prob", 1.0)),
                    "y_candidate": np.asarray(y_c, dtype=np.float64).tolist(),
                })
            ...
            try:
                ...
                save_plot_payload(
                    out_png,
                    payload={
                        "plot_kind": "ml_hist_grid_v1",
                        "title": f"{plot_title} | gate={gate_tag} | {beta_tag}",
                        "active_bins": int(n_active),
                        "q_active_min": float(q_active_min),
                        "q_active_max": float(q_active_max),
                        "rows": int(rows),
                        "cols": int(cols),
                        "candidates": candidate_curves,
                    },
                    arrays={
                        "x_active": x,
                        "y_residual": y_res,
                ...
                print(f"[ml-plot] payload save warning: {payload_err}")
